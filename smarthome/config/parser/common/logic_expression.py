...
def parse_logic_expression(expression):
    ...
    return LogicExpression(expression=lambda object_manager, **kwargs: eval(code, {},
                                                                            LogicExpressionContext(object_manager, dict(
                                                                                dict(os=os,
                                                                                     DATA_DIR=DATA_DIR,
...
class LogicExpressionContext(object):
    ...
    def __getitem__(self, item):
        ...
        return LogicExpressionObjectProxy(self.object_manager, item)


class LogicExpressionObjectProxy(object):
    def __init__(self, object_manager, name):
        self.object_manager = object_manager
        self.name = name

    def __getattr__(self, item):
        return self.object_manager.objects[self.name].get_property(item)
